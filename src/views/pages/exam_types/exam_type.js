...
import { getExamTypeListApi, deleteExamTypeApi } from '../../../api/exam_types';
...
const ExamType = () => {
    ...
    const Pagenavigate = (value, id) => {
        navigate("/exam_typeForm", {
            state: { value, id },
        });
    }

    const { showNotification, setExamData, setPermissionData } = commonDispatch();
    ...
    const getExamTypeList = async (filterValue) => {
        let response = await getExamTypeListApi(filterValue ?? defaultFilter);
        if (response && response?.status === 200) {
            setExamTypeList(response?.data?.data?.rows);
            setExamData(response?.data?.data?.rows)
